import { PROFILE_TYPES, getJurisdiction, jurisdictionName } from 
"@/lib/complianceContent";

// ---- Expiry alert engine (reusable): alerts at 30/14/7 days, escalation at 0 ----
export const EXPIRY_LEVELS = [
 { key: "valid", minDays: 31, label: "Valid", color: "bg-emerald-100  text-emerald-700" },
 { key: "30d", minDays: 15, label: "30-day alert", color: "bg-sky-100  text-sky-700" },
 { key: "14d", minDays: 8, label: "14-day alert", color: "bg-amber-100  text-amber-700" },
 { key: "7d", minDays: 1, label: "7-day alert", color: "bg-orange-100  text-orange-700" },
 { key: "expired", minDays: -Infinity, label: "EXPIRED — escalated to  compliance officer", color: "bg-rose-100 text-rose-700" }
];
...
export const buildDocumentChecklist = (profileType, jurisdictions = [], 
crossBorder = false) => {
 ...
 items.push({ ...d, id: `${d.id}::${j}`, label: `${d.label}`, jurisdiction: j, 
...
export const buildDviChecklist = (profileType, crossBorder = false) =>
 (PROFILE_TYPES[profileType]?.dvi || []).filter((i) => !i.crossBorderOnly || 
crossBorder);

// ---- Dispatch gate (reusable rule engine) ----
export const parseChecklist = (data) =>
 Array.isArray(data) ? data.map((s) => { try { return JSON.parse(s); } catch { 
...
export const evaluateGate = (documentsData) => {
 ...
 const failures = items.filter((i) => !i.required === false && 
BLOCKING_STATUSES.includes(i.status));
...
export const registerNumber = (existingCount) =>
 `CC-INC-${new Date().getFullYear()}-${String(existingCount + 1).padStart(4, 
"0")}`;
