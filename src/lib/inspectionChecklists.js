...
export const getInspectionSections = (
  ...
  const combo = combinationMeta(combinationType);
  const fleetSpecific = FLEET_CHECKS[fleetType] || {
    title: "Fleet-Specific",
    items: [],
  };
  const tyreItems =
    tyreItemsOverride && tyreItemsOverride.length > 0
      ? tyreItemsOverride
      : getTyreCheckItems(combinationType);

  const sections = [
    {
      id: "truck_cab",
      title: "Truck — Cab & Documents",
      items: TRUCK_CAB_CHECKS,
    },
    {
      id: "truck_external",
      title: "Truck — External & Mechanical",
      items: TRUCK_EXTERNAL_CHECKS,
    },
  ];
  ...
  sections.push({
    id: "fleet_specific",
    title: fleetSpecific.title,
    items: fleetSpecific.items,
  });
