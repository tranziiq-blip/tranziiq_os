...
const DUAL_PATTERN = ["Outer Left", "Inner Left", "Inner Right", "Outer  Right"];
...
export function generateTruckPositionNames(count) {
 const names = [];
 for (let i = 0; i < Math.min(count, 2); i++) names.push(`Steer Axle — 
${SINGLE_PATTERN[i]}`);
 let remaining = count - names.length;
 let axle = 1;
 while (remaining > 0 && axle <= 10) {
 const pattern = remaining >= 4 ? DUAL_PATTERN : SINGLE_PATTERN.slice(0, 
...
export function defaultTruckTyrePositions(combinationType) {
 ...
 const truckAxles = combo.axles.filter((a) => !a.id.startsWith("trailer") && 
!a.id.startsWith("link"));
...
export function defaultTrailerTyrePositions(axleCount, tyreType) {
 const pattern = tyreType === "super_single" ? SINGLE_PATTERN : DUAL_PATTERN;
 const names = [];
 for (let ax = 1; ax <= (axleCount || 2); ax++) pattern.forEach((p) => 
names.push(`Axle ${ax} — ${p}`));
 return names;
}
...
export function getCombinedPositions(truck, trailer) {
 ...
 : [];
 return [...horse.map((p) => `Horse — ${p}`), ...trailerPos.map((p) => 
`Trailer — ${p}`)];
}
