...
export default function Production() {
  ...
  const inTransit = loads.filter((l) =>
    [
      ...
      "loading",
      "weighing_out_loaded",
      "enroute_to_offloading",
      "arrived_at_offloading",
      "queue_to_offload",
      "weighing_in_loaded",
      "offloading",
      "weighing_out_empty",
  ...
  const completed = loads.filter((l) =>
    ["load_completed", "delivered", "pod_captured", "completed"].includes(
      l.status,
    ),
  );
  const activeTrucks = trucks.filter((t) => t.status === "active").length;
  const utilisation = trucks.length
    ? Math.round((inTransit.length / Math.max(activeTrucks, 1)) * 100)
    : 0;

  const avgTAT = completed.length
    ? completed.reduce((sum, l) => {
        const start = l.pickup_date
          ? new Date(l.pickup_date).getTime()
          : new Date(l.created_date).getTime();
        const end = l.delivery_date
          ? new Date(l.delivery_date).getTime()
          : new Date(l.created_date).getTime();
        return sum + Math.max(hrsBetween(start, end), 0);
      }, 0) / completed.length
    : 0;

  const crossBorderLoads = loads.filter(
    (l) =>
      l.cross_border &&
  ...
  const kpis = [
    {
      ...
      sub: `${loads.length} total`,
      icon: Activity,
      tone: "text-brand-blue",
    },
    {
      label: "Avg Turnaround",
      value: loading ? "—" : `${avgTAT.toFixed(1)}h`,
      sub: "completed loads",
      icon: Timer,
    ...
    },
    {
      ...
      value: loading ? "—" : `${Math.min(utilisation, 100)}%`,
      sub: `${activeTrucks} active trucks`,
      icon: Gauge,
    ...
    },
    {
      ...
      sub: `${fatigueFlags.length} rest due`,
      icon: Users,
      tone: "text-amber-500",
    },
  ];
    ...
    <div className="space-y-6 animate-fade-in">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Operations · Driver performance · Truck performance · Fuel management
        </p>
      </div>
        ...
        <TabsContent value="operations" className="space-y-6">
            ...
            <Card className="border-border/60 shadow-sm">
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                <CardTitle className="flex items-center gap-2 text-base  font-semibold">
                  <Coffee size={16} className="text-amber-500" /> Fatigue
                  Oversight
                </CardTitle>
                {fatigueFlags.length > 0 && (
                  <Badge variant="destructive">
                    {fatigueFlags.length} rest due
                  </Badge>
                )}
              </CardHeader>
              <CardContent>
                <div className="space-y-2">
                  {shifts.map((s) => {
                    ...
                    const restDue =
                      elapsed >= 2 * 3600000 && (s.rests_taken || 0) === 0;
                      ...
                      >
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            {s.truck_registration || "Unassigned"} · {hrs}h on
                            shift · {s.rests_taken || 0} rests
                          </p>
                        </div>
                        {overLimit ? (
                          <Badge variant="destructive">
                          ...
                          <Badge variant="destructive">Rest due</Badge>
                        ) : (
                          <Badge
                            variant="secondary"
          ...
          <Card className="border-border/60 shadow-sm">
            <CardHeader className="pb-2">
              <CardTitle className="flex items-center gap-2 text-base  font-semibold">
                <Activity size={16} className="text-brand-blue" /> Live Load
                Monitoring
              </CardTitle>
            </CardHeader>
                    ...
                    return (
                      ...
                      >
                        <td className="px-4 py-3 font-semibold  text-brand-navy">
                          {l.load_number}
                          {l.cross_border && (
                            <span className="ml-1.5  inline-flex rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-medium  text-amber-700">
                              Cross-Border
                            </span>
                          )}
                        </td>
...
font-medium ${meta.color}`}
                        ...
                        <td className="px-4 py-3">
                          <span
                            className={`flex items-center gap-1 font-medium ${
                              tatRisk ? "text-rose-600" : "text-foreground"
                            }`}
                          >
                            {tatRisk && <AlertTriangle size={13} />}
                            {elapsedHrs}h
                          </span>
                        </td>
                        <td className="px-4 py-3">
          ...
          {crossBorderLoads.length > 0 && (
            <Card className="border-amber-200 bg-amber-50/50">
              <CardHeader className="pb-2">
                <CardTitle className="flex items-center gap-2 text-base font-semibold">
                  <Globe size={16} className="text-amber-600" /> Cross-Border
                  Freight Clearance
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-2">
                <p className="text-xs text-muted-foreground">
                  Cross-border loads require coordination between driver,
                  operations, and freight clearance portal.
                </p>
                {crossBorderLoads.map((l) => (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {l.origin} → {l.destination} via
                        {l.border_post || "—"}
                      </p>
                      {l.freight_forwarder && (
                        <p className="text-xs  text-muted-foreground">
                          Forwarder: {l.freight_forwarder}
                        </p>
                      )}
                    </div>
...
${
  l.customs_status === "cleared"
    ? "bg-emerald-100 text-emerald-700"
    : l.customs_status === "held"
      ? "bg-rose-100 text-rose-700"
      : "bg-amber-100  text-amber-700"
}`}
                      >
                        {(l.customs_status || "pending").replace(/_/g, " ")}
                      </span>
                    </div>
                  </div>
                ))}
          ...
          <Card className="border-border/60 shadow-sm">
            <CardHeader className="pb-2">
              <CardTitle className="flex items-center gap-2 text-base  font-semibold">
                <TrendingUp size={16} className="text-brand-teal" /> Fleet
                Utilisation by Type
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {Object.entries(
                  trucks.reduce((acc, t) => {
                    ...
                    acc[key].total++;
                    if (inTransit.some((l) => l.truck_id === t.id))
                      acc[key].working++;
                    return acc;
                  }, {}),
                ).map(([key, v]) => {
                  const pct = v.total
                    ? Math.round((v.working / v.total) * 100)
                    : 0;
                  return (
                    <div
                      key={key}
