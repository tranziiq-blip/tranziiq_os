...
export default function Fuel() {
  ...
  useEffect(() => {
    if (driver)
      base44.entities.FuelLog.filter(
        { driver_id: driver.id },
        "-created_date",
        5,
      ).then(setRecent);
  ...
  const submit = async () => {
    if (!truckId || !litres) {
      toast({ title: "Truck & litres required", variant: "destructive" });
      return;
    }
    try {
      ...
      if (driver)
        setRecent(
          await base44.entities.FuelLog.filter(
            { driver_id: driver.id },
            "-created_date",
            5,
          ),
...
gap-2 rounded-lg border-2 border-dashed border-border p-6 ${
              ...
              >
                ...
                <span className="text-xs text-muted-foreground">
                  {uploading ? "Uploading…" : "Tap to capture slip"}
                </span>
                <input
                  type="file"
                  accept="image/*"
