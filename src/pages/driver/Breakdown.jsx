...
export default function Breakdown() {
 ...
 useEffect(() => { if (driver) base44.entities.BreakdownReport.filter({ 
driver_id: driver.id }, "-created_date", 5).then(setRecent); }, [driver?.id]);
...
"destructive" });
 ...
 const submit = async () => {
 if (!truckId || !description) { toast({ title: "Truck & description  required", variant: "destructive" }); return; }
 try {
 ...
 description,
 status: "open",
 priority: severity === "critical" ? "critical" : severity === "high" ? "high" 
: "medium",
 asset_type: "truck"
 });
 }
 toast({ title: "Report submitted", description: reportType === "breakdown" ? 
"Job card auto-created for engineering" : undefined });
 setDescription(""); setLocation(""); setPhotoUrl(""); setTruckId("");
 if (driver) setRecent(await base44.entities.BreakdownReport.filter({ 
driver_id: driver.id }, "-created_date", 5));
 } catch (e) {
 ...
 <div>
 <h1 className="font-display text-xl font-bold text-brand-navy">Report 
Issue</h1>
 <p className="text-sm text-muted-foreground">Breakdown, incident or 
near-miss</p>
 </div>
 ...
 {TYPES.map((t) => (
 <button key={t.key} onClick={() => setReportType(t.key)} className={`flex 
flex-col items-center gap-1 rounded-lg border p-3 transition ${reportType === 
t.key ? "border-brand-teal bg-brand-teal/5 text-brand-teal" : "border-border  bg-card text-muted-foreground"}`}>
 <t.icon size={20} />
...
value={t.id}>{t.registration_number}</SelectItem>)}</SelectContent>
 ...
 {SEVERITY.map((s) => (
 <button key={s.key} onClick={() => setSeverity(s.key)} className={`flex-1 
rounded-lg border py-2 text-xs font-medium transition ${severity === s.key ? 
`border-current ${s.color} bg-muted` : "border-border  text-muted-foreground"}`}>{s.label}</button>
 ))}
 </div>
 </div>
 <div className="grid gap-2">
...
setDescription(e.target.value)} rows={3} placeholder="Describe what happened…" 
/>
 </div>
 <div className="grid gap-2">
 <Label>Photo</Label>
...
onChange={(e) => uploadPhoto(e.target.files[0])} />
 ...
 {recent.length > 0 && (
 <div>
 <p className="mb-2 text-xs font-semibold uppercase tracking-wider  text-muted-foreground">Recent Reports</p>
 <div className="space-y-2">
 {recent.map((r) => (
 <Card key={r.id} className="border-border/60">
 <CardContent className="p-3">
 <div className="flex items-center justify-between">
 <span className="text-sm font-semibold  capitalize">{r.report_type.replace(/_/g, " ")}</span>
 <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold 
${r.status === "open" ? "bg-rose-100 text-rose-700" : r.status === 
"in_progress" ? "bg-amber-100 text-amber-700" : "bg-emerald-100  text-emerald-700"}`}>{r.status.replace(/_/g, " ")}</span>
 </div>
 <p className="mt-1 text-xs text-muted-foreground">{r.truck_registration} · 
{r.location}</p>
