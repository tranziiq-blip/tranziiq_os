...
const READING_TYPES = [
  { key: "pre_cool", label: "Pre-Cool" },
  { key: "pickup", label: "Pickup" },
  { key: "in_transit", label: "In Transit" },
  { key: "checkpoint", label: "Checkpoint" },
  { key: "border", label: "Border Crossing" },
...
export default function TemperatureLogDialog({
  ...
  const load = async () => {
    ...
    const [r, e] = await Promise.all([
      base44.entities.TemperatureReading.filter(
        { load_id: profile.load_id },
        "recorded_at",
        200,
      ),
      base44.entities.TemperatureExcursion.filter(
        { load_id: profile.load_id },
        "-started_at",
        50,
      ),
...
excursion opened`,
          ...
          toast({
            title: "THRESHOLD BREACH — excursion opened",
            description: `Alert 
raised to the compliance officer. Excursion report auto-generates if breach 
exceeds ${profile.logger_interval_minutes || 15} minutes.`,
            variant: "destructive",
          });
        } else {
          await base44.entities.TemperatureExcursion.update(openExc.id, {
            min_temp_c: Math.min(openExc.min_temp_c ?? temp, temp),
            max_temp_c: Math.max(openExc.max_temp_c ?? temp, temp),
          });
          toast({
            title: "THRESHOLD BREACH — excursion continues",
            variant: "destructive",
          });
        }
      } else {
        ...
        if (openExc) {
          ...
          await base44.entities.TemperatureExcursion.update(openExc.id, {
            ...
            status: "closed",
            excursion_report_required:
              duration > (profile.logger_interval_minutes || 15),
            severity:
              duration > (profile.logger_interval_minutes || 15)
                ? "high"
                : "medium",
          ...
          toast({
            title: "Temperature back in range — excursion closed",
            description: `Breach duration: ${duration} min${
              duration > (profile.logger_interval_minutes || 15)
                ? " — Cold-Chain Excursion Report required"
                : ""
            }`,
          });
        } else {
          toast({
            ...
            description: `${temp}°C — within range`,
          });
        }
      }
      setForm({
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Thermometer size={16} />
            Cold-Chain Log — {profile.load_number}
          </DialogTitle>
          <p className="text-xs font-normal text-muted-foreground">
            Required range
            {profile.temp_min_c ?? "—"}°C to {profile.temp_max_c ?? "—"}°C ·
            logger
            {profile.logger_serial || "—"} · interval{" "}
            {profile.logger_interval_minutes || 15} min
          </p>
        </DialogHeader>
        ...
        <div className="space-y-4 py-1">
          {readings.length > 0 ? (
            <div className="h-48 rounded-lg border border-border/60 p-2">
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  {profile.temp_min_c != null && (
                    <ReferenceLine
                      y={profile.temp_min_c}
                      stroke="#f97316"
                      strokeDasharray="4 4"
                    />
                  )}
                  {profile.temp_max_c != null && (
                    <ReferenceLine
                      y={profile.temp_max_c}
                      stroke="#f97316"
                      strokeDasharray="4 4"
                    />
                  )}
            ...
            <p className="text-center text-xs text-muted-foreground py-6">
              No readings logged yet — capture the pre-cool and pickup readings
              before dispatch.
            </p>
          )}
                ...
                .map((r) => (
                  ...
                  >
                    <span className="text-muted-foreground">
                      ...
                      {READING_TYPES.find((t) => t.key === r.reading_type)
                        ?.label || r.reading_type}
          ...
          {excursions.length > 0 && (
            <div className="rounded-lg border border-rose-200 bg-rose-50 p-3">
              <p className="mb-1.5 text-xs font-semibold uppercase tracking-wider  text-rose-700">
                Excursions
              </p>
              {excursions.map((e) => (
                <p key={e.id} className="text-[11px] text-rose-700">
                  ...
                  · {e.min_temp_c}–{e.max_temp_c}°C
                  {e.excursion_report_required
                    ? "· Excursion Report REQUIRED"
                    : ""}
                </p>
              ))}
            </div>
          ...
          <div className="grid grid-cols-2 gap-3 rounded-lg border border-border/60 p-3  sm:grid-cols-4">
            ...
            <div className="grid gap-1.5">
              ...
              <Select
                ...
                onValueChange={(v) => setForm({ ...form, reading_type: v })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <div className="flex items-end">
              <Button
                onClick={addReading}
                disabled={saving}
                className="w-full gap-1.5 bg-brand-teal  hover:bg-brand-teal/90"
              >
                <Plus size={14} /> Log
              </Button>
            </div>
