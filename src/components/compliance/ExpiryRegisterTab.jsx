...
export default function ExpiryRegisterTab({ documents, onRefresh }) {
  ...
  const list = documents.filter(
    (d) => filter === "all" || d.profile_type === filter,
  );
  const expired = documents.filter(
    (d) => expiryLevel(d).key === "expired",
  ).length;
        ...
        <CardContent className="p-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Alerts at 30 / 14 / 7 days before expiry — escalates to the
                compliance officer at 0 days
              </p>
            </div>
          ...
          <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
            <div className="rounded-lg border border-rose-200 bg-rose-50 p-2  text-center">
              ...
              <p className="text-[10px] text-rose-700">Expired — escalated</p>
            </div>
            <div className="rounded-lg border border-orange-200 bg-orange-50 p-2  text-center">
              ...
              <p className="text-[10px] text-orange-700">≤ 7 days</p>
            </div>
            <div className="rounded-lg border border-amber-200 bg-amber-50 p-2  text-center">
              <p className="font-display text-lg font-bold  text-amber-600">
                {d14}
              </p>
              <p className="text-[10px] text-amber-700">≤ 14 days</p>
            </div>
            <div className="rounded-lg border border-sky-200 bg-sky-50 p-2  text-center">
              <p className="font-display text-lg font-bold  text-sky-600">
                {d30}
              </p>
              <p className="text-[10px] text-sky-700">≤ 30 days</p>
            </div>
          </div>
        </CardContent>
      </Card>
            ...
            <TableBody>
              {list.length === 0 && (
                <TableRow>
                  ...
                  >
                    No tracked documents yet — add expiry-tracked certificates,
                    permits and service records.
                  </TableCell>
                </TableRow>
              )}
              {list.map((d) => {
                  ...
                  <TableRow key={d.id}>
                    ...
                    <TableCell>
                      <Badge variant="secondary" className="text-[10px]">
                        {PROFILE_TYPES[d.profile_type]?.label || d.profile_type}
                      </Badge>
                    </TableCell>
