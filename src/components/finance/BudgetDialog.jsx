...
export default function BudgetDialog({ open, onOpenChange, editing, onSaved }) {
  ...
  const save = async () => {
    if (!form.budget_name || !form.period_month) {
      toast({ title: "Name & period  required", variant: "destructive" });
      return;
    }
    setSaving(true);
    try {
      const payload = {
        ...
        budgeted_distance_km: Number(form.budgeted_distance_km) || 0,
        budgeted_cpk:
          (Number(form.budgeted_amount) || 0) /
          (Number(form.budgeted_distance_km) || 1),
      };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>
            {editing?.id ? "Edit Budget" : "Create  Budget"}
          </DialogTitle>
        </DialogHeader>
        <div className="grid grid-cols-2 gap-3 py-1">
          <F label="Budget Name *" full>
            <Input
              value={form.budget_name}
              onChange={(e) => set("budget_name", e.target.value)}
              placeholder="e.g. September Fuel Budget"
            />
          </F>
          <F label="Category">
            <Select
          ...
          <F label="Budgeted Distance (km)">
            <Input
              type="number"
              value={form.budgeted_distance_km}
              onChange={(e) => set("budgeted_distance_km", e.target.value)}
            />
          </F>
          <F label="Notes" full>
            <Textarea
