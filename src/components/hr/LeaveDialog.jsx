...
const empty = { employee_id: "", leave_type: "annual", start_date: "", 
end_date: "", days_requested: 1, reason: "", status: "pending", approved_by: 
"", notes: "" };

export default function LeaveDialog({ open, onOpenChange, employees, onSaved 
}) {
 ...
 const save = async () => {
 if (!form.employee_id || !form.start_date || !form.end_date) { toast({ title: 
"Employee & dates required", variant: "destructive" }); return; }
 const emp = employees.find((e) => e.id === form.employee_id);
 const days = Math.max(1, Math.ceil((new Date(form.end_date) - new 
Date(form.start_date)) / 86400000) + 1);
 setSaving(true);
 try {
 await base44.entities.LeaveApplication.create({ ...form, days_requested: 
days, employee_name: emp?.full_name || "", employee_number: 
emp?.employee_number || "", department: emp?.department || "" });
...
{e.job_title}</SelectItem>)}</SelectContent></Select></F>
 <F label="Leave Type"><Select value={form.leave_type} onValueChange={(v) => 
set("leave_type", v)}><SelectTrigger><SelectValue 
/></SelectTrigger><SelectContent>{LEAVE_TYPES.map((t) => <SelectItem 
...
/></SelectTrigger><SelectContent>{Object.entries(LEAVE_STATUS).map(([k, s]) => 
<SelectItem key={k} 
...
onChange={(e) => set("start_date", e.target.value)} /></F>
 <F label="End Date *"><Input type="date" value={form.end_date} onChange={(e) => set("end_date", e.target.value)} /></F>
 <F label="Approved By"><Input value={form.approved_by} onChange={(e) => 
set("approved_by", e.target.value)} placeholder="Manager name" /></F>
 <div className="col-span-2" />
 <F label="Reason" full><Textarea value={form.reason} onChange={(e) => 
set("reason", e.target.value)} rows={2} /></F>
