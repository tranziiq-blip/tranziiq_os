    ...
    (async () => {
      try {
        ...
        const result = d.map((driver) => {
          ...
          const events = dt.reduce(
            (s, t) =>
              s +
              (t.harsh_braking_count || 0) +
              (t.harsh_acceleration_count || 0) +
              (t.speeding_count || 0) +
              (t.harsh_cornering_count || 0),
            0,
          );
          ...
          return {
            ...
            loadsTotal: dl.length,
            completionRate: dl.length
              ? Math.round((completed.length / dl.length) * 100)
              : 0,
            inspectionRate: di.length
              ? Math.round((passed.length / di.length) * 100)
              : 0,
            riskCount: dr.length,
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <Card className="border-border/60 shadow-sm">
          <CardContent className="p-4">
            ...
            <p className="mt-1 font-display text-2xl  font-bold text-brand-navy">
              {rows.reduce((s, r) => s + r.loadsCompleted, 0)}
            </p>
          </CardContent>
        </Card>
        <Card className="border-border/60 shadow-sm">
          <CardContent className="p-4">
            ...
            <p className="mt-1 font-display  text-2xl font-bold text-brand-navy">
              {rows.length
                ? Math.round(
                    rows.reduce((s, r) => s + r.inspectionRate, 0) /
                      rows.length,
                  )
                : 0}
              %
        ...
        <Card className="border-border/60 shadow-sm">
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <p className="text-xs text-muted-foreground">Behavior Events</p>
              <Activity size={16} className="text-amber-500" />
            </div>
            <p className="mt-1 font-display  text-2xl font-bold text-brand-navy">
              {rows.reduce((s, r) => s + r.behaviorEvents, 0)}
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="border-border/60 shadow-sm">
        <CardHeader className="pb-2">
          <CardTitle className="flex items-center gap-2  text-base font-semibold">
            <Users size={16} className="text-brand-navy" /> Driver Performance
            Scores
          </CardTitle>
        </CardHeader>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Inspection Pass</TableHead>
                  <TableHead>Risk Assess.</TableHead>
                  <TableHead>Behavior Events</TableHead>
...
${scoreColor(r.score)}`}
      ...
      <p className="text-xs text-muted-foreground">
        Behavior metrics (harsh braking, acceleration, speeding, cornering)
        sourced from telematics API. Connect a telematics provider to populate
        driver scores automatically.
      </p>
    </div>
  );
}
