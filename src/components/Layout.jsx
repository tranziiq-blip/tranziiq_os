...
import {
 LayoutDashboard, Truck, Users, Package, Smartphone, CalendarClock,
 Wrench, Boxes, UserCog, ShieldCheck, Wallet, Settings, Sparkles,
 FileText, Globe, Plug, CreditCard, LogOut, Menu, X, ChevronRight,
 ChevronDown, Scale, ClipboardCheck, Flame
} from "lucide-react";
...
const ACTIVE = [
 ...
 { to: "/production", label: "Production", icon: CalendarClock, children: [
 { to: "/fleet", label: "Fleet Register", icon: Truck },
 { to: "/loads", label: "Dispatch", icon: Package }
 ]},
 { to: "/engineering", label: "Engineering", icon: Wrench },
 { to: "/safety", label: "SHEQ", icon: ShieldCheck, children: [
 { to: "/compliance", label: "Compliance", icon: ClipboardCheck }
 ]},
 { to: "/compliance-ops", label: "Specialized Compliance", icon: Flame, 
requiresAddon: "compliance" },
 { to: "/stores", label: "Stores", icon: Boxes },
 { to: "/hr", label: "HR", icon: UserCog, children: [
...
export default function Layout() {
 ...
 useEffect(() => {
 base44.entities.CompanyProfile.list().then((cp) => {
 const addons = cp[0]?.selected_addons || [];
 setSpecializedActive(["dg_hazmat", "cold_chain", "abnormal_load"].some((a) => 
addons.includes(a)));
 }).catch(() => {});
 }, []);
...
""}`} size={14} />
 ...
 const visibleNav = ACTIVE.filter((item) => userHasAccess(user, 
item.to)).filter((item) => !item.requiresAddon || specializedActive);
 ...
 <p className="px-3 pb-2 text-[10px] font-semibold uppercase tracking-wider  text-sidebar-foreground/50">Operations</p>
 ...
 <p className="truncate text-sm font-medium text-white">{user?.full_name || 
"Operator"}</p>
 <p className="truncate text-xs text-sidebar-foreground/60">{user?.email}</p>
 </div>
 <button onClick={handleLogout} className="rounded-md p-2  text-sidebar-foreground/70 hover:bg-sidebar-accent hover:text-white" 
title="Sign out">
 <LogOut size={16} />
 </button>
 </div>
