...
const STATUS_FLOW = ["reported", "investigating", "action_pending", "resolved"];
const STATUS_META = {
  reported: { label: "Reported", color: "bg-rose-100  text-rose-700" },
  investigating: {
    label: "Investigating",
    color: "bg-amber-100  text-amber-700",
  },
  action_pending: { label: "Action Pending", color: "bg-sky-100 text-sky-700" },
...
export default function IncidentsTab({
  ...
  const save = async () => {
    ...
    await base44.entities.IncidentReport.create({
      ...
      truck_registration: truckReg(form.truck_id),
      status: "reported",
    });
  ...
  const advance = async (i) => {
    ...
    await base44.entities.IncidentReport.update(i.id, updates);
    toast({
      title: `Status: 
${STATUS_META[next].label}`,
  ...
  const saveInv = async () => {
    await base44.entities.IncidentReport.update(investigate.id, {
      root_cause: invForm.root_cause,
      corrective_action: invForm.corrective_action,
      investigated_by: invForm.investigated_by,
      status:
        investigate.status === "reported"
          ? "investigating"
          : investigate.status,
    });
              ...
              {filtered.map((i) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <Badge className={TYPE_META[i.incident_type]}>
                      {i.incident_type.replace("_", "  ")}
                    </Badge>
                  </td>
                  <td className="px-4 py-3">
                    <Badge className={SEVERITY[i.severity]}>{i.severity}</Badge>
                  </td>
...
${STATUS_META[i.status].color}`}
      ...
      <Dialog
        open={!!investigate}
        onOpenChange={(o) => !o && setInvestigate(null)}
      >
        <DialogContent>
          <DialogHeader>
            ...
            <div className="space-y-4 py-2">
              <div className="rounded-lg bg-muted/50 p-3">
                <div className="flex flex-wrap  items-center gap-2">
                  <Badge className={TYPE_META[investigate.incident_type]}>
                    {investigate.incident_type.replace("_", " ")}
                  </Badge>
                  <Badge className={SEVERITY[investigate.severity]}>
                    {investigate.severity}
                  </Badge>
                  <span className="text-xs text-muted-foreground">
                    {investigate.driver_name} ·{investigate.truck_registration}
                  </span>
                </div>
              ...
              <div className="grid gap-2">
                ...
                <Textarea
                  ...
                  onChange={(e) =>
                    setInvForm({ ...invForm, root_cause: e.target.value })
                  }
                  rows={3}
                  placeholder="Identify the underlying  cause(s)…"
                />
              </div>
              <div className="grid gap-2">
                ...
                <Textarea
                  ...
                  rows={3}
                  placeholder="Action taken /  required to prevent recurrence…"
                />
              </div>
              <div className="grid gap-2">
                <Label>Investigated By</Label>
                <Input
                  value={invForm.investigated_by}
                  onChange={(e) =>
                    setInvForm({ ...invForm, investigated_by: e.target.value })
                  }
                />
              </div>
            </div>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setInvestigate(null)}>
              Close
            </Button>
            <Button
              onClick={saveInv}
