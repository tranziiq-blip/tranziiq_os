...
export default function ConnectIntegrationDialog({
  open,
  onOpenChange,
  provider,
  existing,
  ...
  const save = async () => {
    ...
    try {
      if (existing) {
        await base44.entities.IntegrationConfig.update(existing.id, {
          ...form,
  ...
  const needsBaseUrl =
    ["geotab", "erp", "accounting"].includes(provider.category) ||
    provider.name === "Geotab";
  const needsAccountId = ["telematics", "fuel_card"].includes(
    provider.category,
  );
        ...
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <div className="flex h-8 w-8 items-center justify-center rounded-lg  bg-brand-navy/10">
              {provider.icon ? (
                <provider.icon size={16} className="text-brand-navy" />
              ) : (
        ...
        <div className="space-y-3 py-2">
          ...
          <div className="rounded-lg border border-amber-200 bg-amber-50 p-3">
            <p className="text-xs text-amber-800">
              Enter your {provider.name} API credentials below. You can find
              these in your {provider.name} account settings or developer
              portal.
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="grid gap-1.5">
              ...
              <Select
                ...
                onValueChange={(v) => setForm({ ...form, sync_frequency: v })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
