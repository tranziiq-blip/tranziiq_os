...
export default function TyrePositionsEditor({
  value,
  onChange,
  generateNames,
}) {
    ...
    <div className="rounded-lg border border-border/60 p-3">
      <div className="mb-2 flex items-center justify-between">
        <p className="flex items-center gap-1.5 text-xs font-semibold uppercase  tracking-wider text-muted-foreground">
          <CircleDot size={14} className="text-brand-teal" /> Tyre Positions (
          {count})
        </p>
      ...
      {count === 0 ? (
        <p className="text-xs text-muted-foreground">
          No tyre positions set — add positions to enable per-position tyre
          inspections.
        </p>
      ) : (
        <div className="grid max-h-44 gap-1.5 overflow-y-auto sm:grid-cols-2">
          {positions.map((p, i) => (
            <div key={i} className="flex items-center gap-1.5">
              <span className="w-6 shrink-0 text-right text-[10px] font-bold  text-muted-foreground">
                {i + 1}
              </span>
              <Input
                value={p}
                onChange={(e) => renamePosition(i, e.target.value)}
