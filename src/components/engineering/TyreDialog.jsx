...
export default function TyreDialog({
  open,
  onOpenChange,
  editing,
  trucks,
    ...
    if (open) {
      if (editing) {
        setForm({
          ...
          tread_depth_mm: editing.tread_depth_mm ?? "",
          pressure_bar: editing.pressure_bar ?? "",
          condition: editing.condition || "good",
          dot_code: editing.dot_code || "",
          fitted_date: editing.fitted_date || "",
          last_checked_date:
            editing.last_checked_date || new Date().toISOString().slice(0, 10),
          cost: editing.cost ?? "",
          status: editing.status || "fitted",
          notes: editing.notes || "",
        });
      } else {
        setForm({
  ...
  const save = async () => {
    ...
    if (!form.position_name) {
      toast({ title: "Select a tyre position", variant: "destructive" });
      return;
    }
    setSaving(true);
    try {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {editing ? "Edit Tyre Record" : "Add Tyre  Record"}
          </DialogTitle>
        </DialogHeader>
        <div className="grid grid-cols-2 gap-3 py-1">
          ...
          <Field label="Position">
            <Select
              ...
              onValueChange={(v) => set("position_name", v)}
              disabled={
                !selectedAsset || (selectedAsset.positions || []).length === 0
              }
            >
              <SelectTrigger>
                <SelectValue
                  placeholder={
                    selectedAsset &&
                    (selectedAsset.positions || []).length === 0
                      ? "No positions on asset"
                      : "Select position"
                  }
                />
              </SelectTrigger>
              <SelectContent>
                {(selectedAsset?.positions || []).map((p) => (
          ...
          <Field label="DOT Code">
            <Input
              value={form.dot_code}
              onChange={(e) => set("dot_code", e.target.value)}
          ...
          <Field label="Tread Depth (mm)">
            <Input
              ...
              onChange={(e) => set("tread_depth_mm", e.target.value)}
              placeholder="12.5"
            />
          </Field>
          ...
          <Field label="Fitted Date">
            <Input
              type="date"
              value={form.fitted_date}
              onChange={(e) => set("fitted_date", e.target.value)}
            />
          </Field>
          <Field label="Last Checked">
            <Input
              ...
              onChange={(e) => set("last_checked_date", e.target.value)}
            />
          </Field>
