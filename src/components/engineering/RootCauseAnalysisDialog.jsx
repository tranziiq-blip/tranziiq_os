...
export default function RootCauseAnalysisDialog({
  open,
  onOpenChange,
  rca,
  onComplete,
  ...
  const slaDeadline = rca.created_date
    ? new Date(new Date(rca.created_date).getTime() + 24 * 3600000)
    : null;
  const hoursLeft = slaDeadline
    ? Math.ceil((slaDeadline - Date.now()) / 3600000)
    : null;
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Search size={18} className="text-brand-navy" /> Root Cause Analysis
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-3">
          ...
          <div className="grid gap-1.5">
            ...
            <Select
              ...
              onValueChange={(v) => set("failure_category", v)}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
          ...
          <div className="grid gap-1.5">
            <Label className="text-xs">Failed Component</Label>
            <Input
              value={form.failure_component}
              onChange={(e) => set("failure_component", e.target.value)}
              placeholder="e.g. Alternator, brake  chamber"
            />
          </div>
          <div className="grid gap-1.5">
          ...
          <div className="grid gap-1.5">
            ...
            <Textarea
              ...
              rows={2}
              placeholder="What  contributed to the failure?"
            />
          </div>
          <div className="grid gap-1.5">
          ...
          <div className="grid gap-1.5">
            <Label className="text-xs">Completed By (name) *</Label>
            <Input
