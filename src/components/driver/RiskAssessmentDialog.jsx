...
export default function RiskAssessmentDialog({
    ...
    (async () => {
      try {
        ...
        const applicable = all.filter(
          (r) =>
            !r.applicable_roles ||
            r.applicable_roles.length === 0 ||
            r.applicable_roles.includes("Driver"),
        );
  ...
  const handleSubmit = async () => {
    ...
    if (form.newRiskFlagged && !form.newRiskDescription) {
      toast({ title: "Describe the new risk", variant: "destructive" });
      return;
    }
    setSaving(true);
    try {
      await base44.entities.ShiftRiskAssessment.create({
        ...
        new_risk_description: form.newRiskFlagged
          ? form.newRiskDescription
          : "",
      });
      toast({
      ...
      <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <ShieldAlert size={18} className="text-brand-navy" /> Shift Risk
            Assessment
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
            ...
            <div className="rounded-lg border border-border/60 p-3">
              ...
              <div className="space-y-1.5">
                {registerRisks.slice(0, 8).map((r) => (
                  ...
                  >
                    <Checkbox
                      checked={acknowledgedRisks[r.id] || false}
                      onCheckedChange={(v) =>
                        setAcknowledgedRisks((p) => ({ ...p, [r.id]: v }))
                      }
                      className="mt-0.5"
                    />
          ...
          <div className="flex items-center justify-between rounded-lg bg-muted/50 px-3  py-2.5">
            ...
            <Switch
              checked={form.routeFamiliar}
              onCheckedChange={(v) => set("routeFamiliar", v)}
            />
          </div>
          <div className="flex items-center justify-between rounded-lg bg-muted/50 px-3  py-2.5">
            <span className="text-sm font-medium">Had adequate rest?</span>
            <Switch
              checked={form.restAdequate}
              onCheckedChange={(v) => set("restAdequate", v)}
            />
          </div>
          <div className="space-y-1.5">
            ...
            <Select
              ...
              onValueChange={(v) => set("vehicleCondition", v)}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
          ...
          <div className="space-y-1.5">
            ...
            <Textarea
              ...
              rows={2}
              placeholder="Describe any route or load hazards…"
            />
          </div>
          <div className="space-y-1.5">
            ...
            <Textarea
              ...
              rows={2}
              placeholder="What will you do to mitigate risks?"
            />
          </div>
          ...
          <div className="rounded-lg border border-amber-200 bg-amber-50/50 p-3">
            <label className="flex items-center justify-between cursor-pointer">
              <span className="flex items-center gap-1.5 text-sm font-medium  text-amber-800">
                <Flag size={14} /> New risk not in register?
              </span>
              <Switch
                checked={form.newRiskFlagged}
                onCheckedChange={(v) => set("newRiskFlagged", v)}
              />
            </label>
            {form.newRiskFlagged && (
              <Textarea
