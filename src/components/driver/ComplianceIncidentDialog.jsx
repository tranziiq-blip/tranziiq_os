...
export default function ComplianceIncidentDialog({
  open,
  onOpenChange,
  activeLoad,
  driver,
  ...
  const typeOptions =
    profile?.profile_type === "dg_hazmat"
      ? DG_TYPES
      : profile?.profile_type === "cold_chain"
        ? CC_TYPES
        : profile?.profile_type === "abnormal_load"
          ? AB_TYPES
          : ["other"];
  ...
  const save = async () => {
    ...
    try {
      ...
      await base44.entities.ComplianceIncident.create({
        ...
        truck_id: activeLoad?.truck_id || "",
        truck_registration: profile?.truck_registration || "",
        driver_id: driver?.id || "",
        driver_name: driver?.full_name || "",
        jurisdiction_code:
          form.jurisdiction_code || (profile?.jurisdictions || [])[0] || "",
      ...
      toast({
        ...
        description:
          "The compliance officer has been alerted. " +
          (form.emergency_services_notified
            ? "Emergency services marked as notified."
            : ""),
        variant:
          form.severity === "critical" || form.emergency_services_notified
            ? "destructive"
            : "default",
      });
      setForm({});
      ...
      <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-rose-600">
            Report Emergency / Deviation
          </DialogTitle>
        </DialogHeader>
        <div className="grid gap-3 py-1">
          <p className="rounded-lg bg-rose-50 px-3 py-2 text-xs text-rose-700">
            {profile?.profile_type === "dg_hazmat"
              ? "Spill / DG incident — an incident  record is created immediately and routed to the compliance officer with the  emergency contact list for the jurisdiction."
              : profile?.profile_type === "cold_chain"
                ? "Temperature breach — a cold-chain  excursion record is created for the compliance officer."
          ...
          <div className="grid gap-1.5">
            ...
            <Select
              value={form.incident_type || typeOptions[0]}
          ...
          <div className="grid gap-1.5">
            <Label className="text-xs">
              Location (road / km marker / nearest town)
            </Label>
            <Input
              value={form.location || ""}
          ...
          {profile?.profile_type === "dg_hazmat" && (
            <label className="flex items-start gap-2 text-xs">
              <input
                type="checkbox"
                checked={!!form.emergency_services_notified}
                onChange={(e) =>
                  setForm({
                    ...form,
                    emergency_services_notified: e.target.checked,
                  })
                }
                className="mt-0.5 h-4 w-4"
              />
              Emergency services notified (activates the jurisdiction contact
              list)
            </label>
          )}
        </div>
